...
function testFocusKw() {
	...
	var focuskw = jQuery.trim(jQuery('#seo_focuskw').val());
	focuskw = focuskw.toLowerCase();

	var url = jQuery('#slug').val();
	...
	var metadesc = jQuery('#seo_metadesc').val();
	var snippet = jQuery('#seo_snippet');
	...
	if (focuskw != '') {
		...
		html += 'Page title: ' + ptest(snippet.find('.title').text(), p) + '<br/>';
		html += 'Page Url: ' + ptest(url, p2) + '<br/>';
		html += 'Content: ' + ptest(jQuery('#content').val(), p) + '<br/>';
		html += 'Meta description: ' + ptest(metadesc, p);
		html += '</p>';
...
function updateTitle(force) {
	...
	var title = '';
	var titleElm = jQuery('#seo_title');
	var titleLengthElm = jQuery('#seo_title-length');
	var divHtml = jQuery('<div />');
	var snippetTitle = jQuery('#seo_snippet').find('.title');
...
function updateDesc() {
	...
	var autogen = false;
	var desc = jQuery.trim(yst_clean(jQuery('#seo_metadesc').val()));
	var color = '#000';
	var divHtml = jQuery('<div />');

	if (desc == '') {
		if ($('desc').val() != '') {
			desc = divHtml.html($('desc').val()).text();
		}
		...
		if (desc == '') {
			...
			if (focuskw != '') {
				...
				if (desc.search(descsearch) != -1 && desc.length > 160) {
					desc = desc.substr(desc.search(descsearch), 160);
				} else {
	...
	if (autogen || desc.length > 160) {
		var space;
		if (desc.length > 160)
			space = desc.lastIndexOf(" ", ( 160 - 3 ));
		else
			space = 160;
	...
	jQuery('#seo_metadesc-length').html(len);
	jQuery('#seo_snippet').find('.desc span.content').css('color', color).html(desc);
	testFocusKw();
}
...
function updateURL() {
	...
	jQuery('#seo_snippet').find('.url').html(url);
	testFocusKw();
}
...
jQuery(document).ready(function () {
    ...
    var descElm = jQuery('#seo_metadesc');
    ...
    jQuery('#seo_metadesc').keyup(function () {
        updateDesc();
    });

    jQuery(document).on('change', '#seo_title', function () {
        updateTitle();
    });
    jQuery(document).on('change', '#seo_metadesc', function () {
        updateDesc();
    });
    ...
    jQuery(document).on('change', '#seo_focuskw', function () {
        var focuskwhelpElm = jQuery('#focuskwhelp');
        if (jQuery('#seo_focuskw').val().search(',') != -1) {
